...
// Get CRM User
$stmt = $db->prepare("SELECT * FROM team_members WHERE id = ? AND company_id = ? AND is_crm = 1 AND deleted_at IS NULL");
...
<div class="flex-1 ml-64 mt-16 p-6">
    <div class="max-w-3xl mx-auto">
        ...
        <form id="editCrmForm" class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="full_name" value="<?= esc($crm['full_name']) ?>" required class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-colors">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Email Address *</label>
                    <input type="email" name="email" value="<?= esc($crm['email']) ?>" required class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-colors">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Mobile</label>
                    <input type="text" name="mobile" value="<?= esc($crm['mobile']) ?>" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-colors">
                </div>
                ...
                <div>
                    ...
                    <select name="role_id" required class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-colors bg-white">
                        ...
                        <?php foreach ($roles as $r): ?>
                            <option value="<?= $r['id'] ?>" <?= $crm['role_id'] == $r['id'] ? 'selected' : '' ?>><?= esc($r['display_name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="department_id" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-colors bg-white">
                        ...
                        <?php foreach ($departments as $d): ?>
                            <option value="<?= $d['id'] ?>" <?= $crm['department_id'] == $d['id'] ? 'selected' : '' ?>><?= esc($d['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <?php
                        $stmtMgr = $db->prepare("SELECT id, full_name, is_crm FROM team_members WHERE company_id = ? AND id != ? AND status = 'active' AND deleted_at IS NULL ORDER BY full_name ASC");
                        $stmtMgr->execute([$company_id, $id]);
                    ...
                    <select name="reporting_manager_id" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-colors bg-white">
                        ...
                        <?php foreach ($managers as $mgr): ?>
                            <option value="<?= $mgr['id'] ?>" <?= $crm['reporting_manager_id'] == $mgr['id'] ? 'selected' : '' ?>><?= esc($mgr['full_name']) ?><?= $mgr['is_crm'] ? ' (CRM)' : '' ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="status" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-colors bg-white">
                        ...
                        <option value="inactive" <?= $crm['status'] === 'inactive' ? 'selected' : '' ?>>Inactive</option>
                        <option value="archived" <?= $crm['status'] === 'archived' ? 'selected' : '' ?>>Archived</option>
                    </select>
                </div>

                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Remarks / Notes</label>
                    <textarea name="remarks" rows="3" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-colors" placeholder="Any special notes or client handling instructions..."><?= esc($crm['remarks'] ?? '') ?></textarea>
                </div>
            </div>
...
$(document).ready(function() {
    $('#editCrmForm').on('submit', function(e) {
        e.preventDefault();

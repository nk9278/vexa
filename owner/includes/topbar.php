<?php
// owner/includes/topbar.php
if (basename($_SERVER['PHP_SELF']) == basename(__FILE__)) die('Direct access denied.');

try {
    $db = Database::getInstance()->getConnection();
    // Fetch basic user profile for header
    $stmt = $db->prepare("SELECT first_name, last_name, profile_image FROM users WHERE id = ?");
    $stmt->execute([$_SESSION['user_id']]);
    $user = $stmt->fetch();
    $initials = substr($user['first_name'] ?? 'U', 0, 1) . substr($user['last_name'] ?? '', 0, 1);
        ...
        <div class="relative ml-3">
            ...
            <div id="profile-menu" class="hidden origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                <div class="px-4 py-2 text-sm text-slate-700 border-b border-slate-100">
                    <div class="font-medium"><?= esc($user['first_name'] . ' ' . $user['last_name']) ?></div>
                    <div class="text-xs text-slate-500">Owner</div>
                </div>
                <a href="<?= BASE_URL ?>owner/profile/" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50" role="menuitem">My Profile</a>
                <a href="<?= BASE_URL ?>owner/settings/" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50" role="menuitem">Company Settings</a>
